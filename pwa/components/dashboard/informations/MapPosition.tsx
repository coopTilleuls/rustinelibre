...
export const MapPosition = ({repairer, uploadRepairer}: MapPositionProps): JSX.Element => {

    const [mapCenter, setMapCenter] = useState<[number, number]>([
        repairer.latitude ? Number(repairer.latitude) : 46.2276,
        repairer.longitude ? Number(repairer.longitude) : 2.2137,
    ]);
    const [shopPosition, setShopPosition] = useState<[number|null, number|null]>([
        repairer.latitude ? Number(repairer.latitude) : null,
        repairer.longitude ? Number(repairer.longitude) : null,
    ]);
    const {pendingRegistration, setPendingRegistration, setErrorMessage, errorMessage} = useContext(RepairerFormContext);
        ...
        <Box
            ...
            <MapContainer
                ...
                <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                    {(shopPosition[0] && shopPosition[1]) && <Marker
                        key={repairer.id}
                        draggable={true}
                        position={[
                            Number(shopPosition[0]),
                            Number(shopPosition[1]),
                        ]}
                        eventHandlers={{
            ...
            <div>
                <Button type="submit" variant="contained" sx={{my: 2}} onClick={uploadPosition}>
                    {!pendingRegistration ? (
