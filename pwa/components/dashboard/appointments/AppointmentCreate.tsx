...
export const AppointmentCreate = (): JSX.Element => {
  ...
  const handleSelectCustomer = (customer: Customer) => {
    ...
    if (customer) {
      handleGetCustomerBikes(customer);
    }
    setTunnelStep('bikes');
  };
            ...
            <Box width={{xs: '100%', md: '50%'}}>
              <FormControl required fullWidth size="small">
                <InputLabel id="bikeType-label">
                  Vélo concerné par la réparation
                </InputLabel>
                <Select
                  label="Vélo concerné par la réparation"
        ...
        {user && tunnelStep == 'confirm' && selectedCustomer && (
          <Stack
            ...
            <Typography align="justify" sx={{mt: 2}}>
              {`Rendez-vous le ${slotDate} avec ${selectedCustomer.firstName} ${selectedCustomer.lastName} `}
            </Typography>
