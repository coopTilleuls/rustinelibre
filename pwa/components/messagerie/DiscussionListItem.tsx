...
const DiscussionListItem = ({discussionGiven, isCustomer}: DiscussionListItemProps): JSX.Element => {
    ...
    const countUnreadMessagesFromDiscussion = async (): Promise<void> => {
        const countUnread = await discussionResource.countUnreadDiscussion(discussion.id.toString(), {});
        setUnreadCounter(countUnread.count)
    }

    const refreshDiscussion = async (): Promise<void> => {
        const refreshDiscussion = await discussionResource.get(discussion['@id'], true);
